...
test('on a wide screen the thread column sits beside the document, five parts to its seven until the user sizes it', async (t) => {
  const { url } = await startServer(t);
  ...
  assert.ok(Math.abs(documentBox.width - threadBox.width * 1.4) <= 2, `document ${documentBox.width} is two fifths wider than the thread pane ${threadBox.width}`);
});
...
test('5.1 each answer opens with a quiet line naming its backend, the model in its tooltip, and the pending line names the backend', async (t) => {
  ...
  assert.equal(await page.locator('.follow-up-question').getAttribute('placeholder'), 'Ask a follow-up…');
