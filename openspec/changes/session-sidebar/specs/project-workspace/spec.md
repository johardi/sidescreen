...
A session whose last turn is removed SHALL leave the sidebar.
...
- **WHEN** the user hovers a turn row, activates the remove control, and activates the confirmation
...
- **WHEN** the user activates the remove control on a turn that has three threads
...
- **WHEN** the only remaining turn of a session is removed
- **THEN** the session no longer appears in the sidebar

#### Scenario: Removing the turn on screen

- **WHEN** the user removes the turn currently shown
