...
export class SidescreenServer {
  ...
  /**
   * @param {{ store: import('../store/store.js').Store, dispatch: Dispatch, env?: NodeJS.ProcessEnv, cwd?: string, log?: (message: string) => void }} options
   */
  ...
  async #apiAddCarryBack(req, res, sessionId) {
    ...
    /** @type {import('../store/carry-back.js').CarryBackEntry|null} */
    let entry = null;
    const state = await this.store.update((latest) => {
